...
- `edge-admin`: network/security edge. WireGuard, Pi-hole, nftables, Suricata IDS, SSH bastion, Wazuh agent.
...
`slurmctld` is **not** placed on `edge-admin`. SchedMD describes `slurmctld` as the central management daemon and recommends single-purpose node roles for production environments. A backup controller can be added later. `slurmdbd` is initially co-located only to keep the first environment small.
