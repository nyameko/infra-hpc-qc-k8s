...
locals {
  nodes = {
    edge_admin = {
      name="edge-admin", network_id=module.network.mgmt_network_id, subnet_id=module.network.mgmt_subnet_id, fixed_ip="10.50.0.10", flavor_name=var.admin_flavor, image_id=var.image_id, security_groups=[module.security.group_ids["edge-admin"]], key_pair=var.ssh_key_name
    }
    hermes = {
...
resource "openstack_networking_floatingip_v2" "edge_admin" {
  pool = var.external_network_name
}

resource "openstack_networking_floatingip_associate_v2" "edge_admin" {
  floating_ip = openstack_networking_floatingip_v2.edge_admin.address
  port_id     = module.compute.port_ids["edge_admin"]
}
